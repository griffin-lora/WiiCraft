...
void game::manage_chunks_around_camera(
    ...
    chunk::map& chunks,
    const block::column_lookups& col_lookups,
    ...
    iterate_positions_in_sphere(chunk_generation_radius, [view, &chunks, &col_lookups, &stored_chunks, &cam_chunk_pos](auto& offset) {
        ...
        if (!chunks.contains(chunk_pos)) {
            ...
            auto [ it, success ] = chunks.insert(std::make_pair<math::vector3s32, chunk>(std::move(chunk_pos), { view, pos }));
            ...
            if (stored_chunk_it != stored_chunks.end()) {
                ...
                generate_blocks(col_lookups, chunk, pos);
            }
