...
void init_region_management(void) {
    ...
    region_voxel_type_arrays = malloc(num_regions * sizeof(*region_voxel_type_arrays));

    memset(region_voxel_type_arrays, 0, num_regions * sizeof(*region_voxel_type_arrays));
    memset(region_render_infos, 0, num_regions * sizeof(*region_render_infos));
    
    region_render_infos->position = (vec3s) {{ 0.0f, 0.0f, 0.0f }};
    generate_region_voxels((s32vec3s) {{ 0, 0, 0 }}, region_voxel_type_arrays);
